<?php
	class ControllerExtensionExtensionModule extends Controller {
		...
		public function index() {
			...
			if (!empty($this->request->get['duplicate']) && $this->validate()) {
				...
				$module = $query->row;
				$new_module_data = json_decode($module['setting'], true);
				...
				$this->response->redirect($this->url->link('extension/module/' . $extension, 'token=' . $this->session->data['token'] . '&module_id=' . $new_module_id, true));
			}
		...
		public function install() {
			...
			if ($this->validate()) {
				...
				// Call install method if it exsits - v2.0 support
		...
		protected function getList() {
			...
			$controller_path = DIR_APPLICATION . 'controller/';
			if ($files) {
				//debug($files);
				...
				foreach ($files as $file) {
					...
					/* show only these payments, if extra payment needed, add into the list */
					if( in_array($extension, HIDDEN_MODULES) ) {
						continue;
					}
					...
					$module_path = str_replace($find, $replace, $file); // debug($module_path);

					$this->load->language($module_path . $extension);
					...
					foreach ($modules as $module) {
						...
						'duplicate' => $this->url->link('extension/extension/module', 'token=' . $this->session->data['token'] . '&duplicate=1&extension=' . $extension . '&module_id=' . $module['module_id'], true),
						'name'      => $module['name'],
						'edit'      => $this->url->link($module_path . $extension, 'token=' . $this->session->data['token'] . '&module_id=' . $module['module_id'], true),
					...
					if( in_array($extension, $extensions) ){
						...
						'edit'      => $this->url->link( $module_path . $extension, 'token=' . $this->session->data['token'], true)
						);
					}
					else{
						$data['extensions'][] = array(
