		...
		foreach ($files as $file) {
			...
			if ($this->user->hasPermission('access', 'extension/extension/' . $extension)) {
				...
				if($extension == 'payment'){
					$total_count = count(ALLOWED_PAYMENTS);
				} else if($extension == 'shipping'){
					$total_count = count(ALLOWED_SHIPPING);
				} else if($extension == 'module'){
					$total_count = count($files) - count(HIDDEN_MODULES);
				} else if($extension == 'total'){
					$total_count = count($files) - count(HIDDEN_ORDER_MODULES);
				} else {
					$total_count = count($files);
				}

				$text = $this->language->get('heading_title') . ' (' . $total_count .')';

				if($extension == 'timeslot'){
					$text = $this->language->get('heading_title');
				}
