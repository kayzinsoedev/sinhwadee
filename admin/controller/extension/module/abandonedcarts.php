...
class ControllerExtensionModuleAbandonedcarts extends Controller {
	...
	public function getabandonedcarts() {
		...
		if (VERSION >= '2.1.0.1') {
            ...
            $this->data['customer_url'] = 'sale/customer/edit';
		}
		...
		$this->response->setOutput($this->load->view($this->modulePath.'/view_abandonedcarts.tpl', $this->data));
    }
	...
	public function sendcustomemail() {
		if (isset($this->request->post) && isset($this->request->post['ABcart_id']) && isset($this->request->post['AB_template_id'])) {	
			...
			$replacements[7] = '<a href="'.$this->getCatalogURL().'index.php?route=' . $this->modulePath . '/removeCart&id='.$this->request->post['ABcart_id'].'">'.$LangVars['text_unsubscribe'].'</a>';
            $replacements[8] = '<a href="'.$this->getCatalogURL().'index.php?route=' . $this->modulePath . '/restoreCart&id='.$this->request->post['ABcart_id'].'">'.$LangVars['text_restorecart'].'</a>';
