...
class ControllerCatalogNews extends Controller {
	...
	public function insert() {
		...
		if (isset($this->request->files['download_file'])) {

					$filename = basename(html_entity_decode($this->request->files['download_file']['name'], ENT_QUOTES, 'UTF-8'));

					$path = DIR_IMAGE. 'catalog/project/media/'. $filename;
					$path = DIR_IMAGE. 'catalog/project/media/';
							if (!file_exists($path)) {
								mkdir($path, 0777, true);
					}

					$file = move_uploaded_file($this->request->files['download_file']['tmp_name'], DIR_IMAGE. 'catalog/project/media/' . $filename);
					if($file == true){
						$file_attach[] = DIR_IMAGE . 'catalog/project/media/'. $filename;
						$file_link[] = $filename;
					}
	...
	if (isset($this->error['warning'])) {
		...
		if (isset($this->request->post['related'])) {
			...
			$data['related'] = $this->model_catalog_news->getNewsDescriptions($this->request->get['news_id']);
		} else {
			$data['related'] = array();
		}
		...
		if (isset($this->request->files['download_file'])) {
			$data['download_file'] = $this->request->files['download_file']['name'];
		}elseif (!empty($news_info)) {
				$data['download_file'] = $news_info['download_file'];
		}else{
				$data['download_file'] = '';
		}
		...
		$recipes_sauce_modulename  = 'recipes_sauces';
		$data['recipes_sauces'] = $Modulehelper->get_field ( $oc, $recipes_sauce_modulename, $language_id, 'sauces');
		// debug($data['recipes_sauces']);
