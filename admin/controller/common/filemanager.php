...
class ControllerCommonFileManager extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->get['ckedialog']) && $this->config->get('ea_cke_enable_ckeditor') == 1) {
	...
	public function connector() {
		...
		if( isset($this->request->get['name']) && strposa($this->request->get['name'], $this->restricted) ){
			$json =  array(
				'added' => array(),
				'warning' => array(
					0 => 'Warning',
					1 => "Folder name cannot contain special characters (Eg: ~ ! @ # $ % ^ & * ( ) \' \")",
					2 => '',
				)
			);
		...
		if (empty($volumes)) { // default settings
			$bits = array(
				array(
					...
					//'uploadAllow'     => array('image', 'application/pdf', 'video/mp4'),
					'uploadAllow'     => array('image', 'application/pdf', 'video/mp4', 'application/vnd.ms-word', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.ms-powerpoint', 'application/vnd.openxmlformats-officedocument.presentationml.presentation', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'),
			...
			foreach ($volumes as $driver => $volume) {
				foreach ($volume as $vkey => $vdata) {
					if ($vdata['status']) {
						...
						//$bits[$i]['uploadAllow']     = array('image', 'application/pdf', 'video/mp4',);
						$bits[$i]['uploadAllow']     = array('image', 'application/pdf', 'video/mp4', 'application/vnd.ms-word', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.ms-powerpoint', 'application/vnd.openxmlformats-officedocument.presentationml.presentation', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
						...
						$bits[$i]['uploadMaxSize']   = '500M';
						$bits[$i]['attributes']	  	 = array(
															array(
																'pattern' => "/\.(?:gitignore|html|php)$/",
																"hidden"  => true
															)
