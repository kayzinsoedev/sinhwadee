...
<div id="content">
	...
	<div class="container-fluid">
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
              ...
              <div class="form-group">
								<label class="col-sm-2 control-label" for="input-blog"><span data-toggle="tooltip" title="<?= $help_related; ?>">Related Blogs</span></label>
								<div class="col-sm-10">
									<input type="text" name="blog_related" value="" placeholder="Related Blogs" id="input-blog" class="form-control" />
									<div id="blog-related" class="well well-sm" style="height: 150px; overflow: auto;">
                          <!-- <?php debug($blog_relateds);?> -->
										<?php foreach ($blog_relateds as $blog_related) { ?>
											<div id="product-related<?= $blog_related['news_id']; ?>"><i class="fa fa-minus-circle"></i> <?= $blog_related['label']; ?>
												<input type="hidden" name="blog_related[]" value="<?= $blog_related['news_id']; ?>" />
											</div>
										<?php } ?>
									</div>
								</div>
    ...
    $('input[name=\'blog_related\']').autocomplete({
			'source': function(request, response) {
				$.ajax({
					url: 'index.php?route=catalog/news/autocomplete&token=<?= $token; ?>&filter_name=' +  encodeURIComponent(request),
					dataType: 'json',
					success: function(json) {
						response($.map(json, function(item) {
              console.log(item);
