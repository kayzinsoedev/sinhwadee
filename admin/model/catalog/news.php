...
class ModelCatalogNews extends Model {
	public function addNews($data, $ifcopy = 0, $file) {
		...
		if(isset($data['receipt_sauces'])){
				foreach ($data['receipt_sauces'] as $key => $value) {
						$this->db->query("INSERT INTO news_recipes_sauces SET news_id = '" . (int)$news_id . "', sauce_id = '" . (int)$value . "'");
				}
		}

		if(isset($data['receipt_cooking_method'])){
				foreach ($data['receipt_cooking_method'] as $key => $value) {
						$this->db->query("INSERT INTO news_recipes_cooking_method SET news_id = '" . (int)$news_id . "', cooking_method_id = '" . (int)$value . "'");
				}
		}
		if(isset($data['receipt_main_ingredient'])){
				foreach ($data['receipt_main_ingredient'] as $key => $value) {
						$this->db->query("INSERT INTO news_recipes_main_ingredients SET news_id = '" . (int)$news_id . "', main_ingredients_id = '" . (int)$value . "'");
				}
		}


		if (isset($file['download_file'])) {
			// debug($file['download_file']['name']);die;
			$this->db->query("UPDATE " . DB_PREFIX . "sb_news SET download_file = '" . $this->db->escape($file['download_file']['name']) . "'  WHERE news_id ='" . (int)$news_id . "'");
		}
	...
	public function editNews($news_id, $data,$file) {
		...
		/*update download pdf file */
		if (isset($data['download_file'])) {
			// debug($data['download_file']);die;
			$this->db->query("UPDATE " . DB_PREFIX . "sb_news SET download_file = '" . $this->db->escape($data['download_file']) . "' WHERE news_id = '" . (int)$news_id . "'");
		}
		/*update download pdf file */
		...
		foreach (@$data['news_description'] as $language_id => $value) {
			$cfield1 = isset($value['cfield1']) ? $value['cfield1'] : null;
			$cfield2 = isset($value['cfield2']) ? $value['cfield2'] : null;
			$cfield3 = isset($value['cfield3']) ? $value['cfield3'] : null;
			$cfield4 = isset($value['cfield4']) ? $value['cfield4'] : null;
			$this->db->query("INSERT INTO " . DB_PREFIX . "sb_news_description SET news_id = '" . (int)$news_id . "', language_id = '" . (int)$language_id . "', title = '" . $this->db->escape($value['title']) . "', description = '" . $this->db->escape($value['description']) . "', meta_desc = '" . $this->db->escape($value['meta_desc']) . "', meta_key = '" . $this->db->escape($value['meta_key']) . "', ctitle = '" . $this->db->escape($value['ctitle']) . "', ntags = '" . $this->db->escape($value['ntags']) . "', description2 = '" . $this->db->escape($value['description2']) . "', cfield1 = '" . $cfield1 . "', cfield2 = '" . $cfield2 . "', cfield3 = '" . $cfield3 . "', cfield4 = '" . $cfield4 . "'");
		}
	...
	public function editRelatedProduct(){
	...
	public function getRelatedNews($news_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "sb_news n LEFT JOIN " . DB_PREFIX . "sb_news_description nd ON (n.news_id = nd.news_id) WHERE n.news_id= '" . $news_id . "' AND nd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY n.date_added");
	...
	public function getRecipesSauces($news_id){
	...
	public function getRecipesIngredients($news_id){
