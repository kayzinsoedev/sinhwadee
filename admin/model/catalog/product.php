...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		if (isset($data['blog_related'])) {
			// debug($data['blog_related']);
			foreach ($data['blog_related'] as $related_id) {
				$this->db->query("DELETE FROM " . DB_PREFIX . "related_blog_product WHERE product_id = '" . (int)$product_id . "' AND news_id = '" . (int)$related_id . "'");
				$this->db->query("INSERT INTO " . DB_PREFIX . "related_blog_product SET product_id = '" . (int)$product_id . "', news_id = '" . (int)$related_id . "'");
			}
		}
		/* Related Blog*/
	...
	public function deleteProduct($product_id) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "related_blog_product WHERE product_id = '" . (int)$product_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "related_blog_product WHERE related_id = '" . (int)$product_id . "'");
	...
	public function getProductRelatedForBlog($news_id) {
		$product_related_blog_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "related_blog_product WHERE news_id = '" . (int)$news_id . "'");

		foreach ($query->rows as $result) {
			$product_related_blog_data[] = $result['product_id'];
		}
	...
	public function getBlogRelated($product_id){
			$product_blog_related_data = array();

			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "related_blog_product WHERE product_id = '" . (int)$product_id . "'");

			foreach ($query->rows as $result) {
				$product_blog_related_data[] = $result['news_id'];
			}

			return $product_blog_related_data;
	}
