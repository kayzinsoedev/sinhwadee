...
class ModelToolProEmail extends Model {
  ...
  private function getDefaultInvoice($order_info) {
    ...
    $data['columns'] = array('image', 'product', 'quantity', 'price');
        ...
        if ($option['type'] != "file") {
          ...
          if ($upload_info) {	
              ...
              if(isset($this->request->get['token']) && $this->request->get['token'] != ''){	
                  $href = $this->url->link('tool/upload/download', '&token='.$this->request->get['token'].'&code=' . $upload_info['code'], true);	
              } else {	
                  $href = $this->url->link('tool/upload/download', '&code=' . $upload_info['code'], true);	
              }	
                  $value = '<a href="'.$href.'">'.$upload_info['name'].'</a>';	
          } else {	
                  $value = '';	
          }
        }
