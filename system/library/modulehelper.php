...
class Modulehelper {
  ...
  public function init ( $array ) {
    ...
    // Remove validation which will cause error
    if (($oc->request->server['REQUEST_METHOD'] == 'POST')) {
 ...
 public function __construct ( $registry ) {
  ...
  public function get_field ( $oc, $modulename, $language_id, $name, $type = false ) {
    ...
    // Pull english content if other language empty
    if($var == '') {
      $var = $oc->config->get($moduleprefix . $modulename . '_' . $name . '_' . 1);
    }
